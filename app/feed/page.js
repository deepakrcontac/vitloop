  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #07070f 0%, #0d0d1a 50%, #07070f 100%)', fontFamily: "'Segoe UI', sans-serif", color: '#fff', paddingBottom: '90px' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '20px 16px' }}>
        <div style={{ background: 'rgba(108,99,255,0.1)', border: '1px solid rgba(108,99,255,0.2)', borderRadius: '12px', padding: '10px 16px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setCategory(cat)} style={{
              ...
              transform: category === cat ? 'translateY(-1px)' : 'none', transition: 'all 0.2s',
            }}>{cat}</button>
          ))}
        </div>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px', color: 'rgba(255,255,255,0.3)' }}>
            <p style={{ fontSize: '32px' }}>⏳</p><p>Loading...</p>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px' }}>
            <p style={{ fontSize: '48px' }}>📦</p>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '15px', marginBottom: '16px' }}>No listings yet.</p>
            <Link href="/sell" style={{ background: 'linear-gradient(135deg, #c8f135, #a8d020)', color: '#0d0d0d', padding: '10px 24px', borderRadius: '10px', textDecoration: 'none', fontWeight: '700', fontSize: '14px' }}>Be the first to list something →</Link>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
            {filtered.map(item => (
              <div key={item.id} style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.07), rgba(255,255,255,0.02))', borderRadius: '16px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)', transition: 'all 0.2s ease', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}
                onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.borderColor = 'rgba(200,241,53,0.3)'; }}
                ...
                <div style={{ height: '130px', background: 'rgba(255,255,255,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px', position: 'relative' }}>
                  ...
                  {isAdmin && (
                    <button
                      ...
                      style={{ position: 'absolute', bottom: '8px', right: '8px', background: 'rgba(255,92,53,0.85)', color: '#fff', border: 'none', borderRadius: '6px', padding: '4px 8px', fontSize: '11px', fontWeight: '700', cursor: 'pointer', backdropFilter: 'blur(4px)' }}
                    >
                      🗑️
                    </button>
                  )}
                ...
                <div style={{ padding: '12px' }}>
                  ...
                  <Link href={`/chat/${item.id}`} style={{ display: 'block', background: 'linear-gradient(135deg, rgba(108,99,255,0.3), rgba(0,212,255,0.2))', border: '1px solid rgba(108,99,255,0.4)', color: '#fff', borderRadius: '8px', padding: '8px', fontSize: '12px', textDecoration: 'none', fontWeight: '700', textAlign: 'center' }}>
                    💬 I'm Interested
                  </Link>
                </div>
              </div>
