    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0d0d0d 0%, #1a1a2e 50%, #0d0d0d 100%)', fontFamily: "'Segoe UI', sans-serif", color: '#fff' }}>
      <nav style={{ padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.08)', position: 'sticky', top: 0, zIndex: 100, background: 'rgba(13,13,13,0.9)', backdropFilter: 'blur(20px)' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)' }}>{auth.currentUser?.email}</span>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '28px 20px' }}>
        <div style={{ background: 'linear-gradient(135deg, rgba(200,241,53,0.08), rgba(255,92,53,0.08))', border: '1px solid rgba(200,241,53,0.2)', borderRadius: '20px', padding: '24px', marginBottom: '32px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
            <input value={newRequest} onChange={e => setNewRequest(e.target.value)} onKeyDown={e => e.key === 'Enter' && postRequest()}
              placeholder="e.g. Need calculator urgently for exam tomorrow 🧮"
            ...
            {requests.map(r => (
              <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.04)', borderRadius: '10px', padding: '12px 16px', border: '1px solid rgba(255,255,255,0.07)' }}>
                <div>
                  ...
                  <p style={{ margin: '4px 0 0', fontSize: '11px', color: 'rgba(255,255,255,0.35)' }}>📧 {r.userEmail}</p>
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <a href={"mailto:" + r.userEmail} style={{ background: 'rgba(200,241,53,0.1)', color: '#c8f135', border: '1px solid rgba(200,241,53,0.2)', borderRadius: '8px', padding: '5px 12px', fontSize: '12px', textDecoration: 'none', fontWeight: '600' }}>
                    💬 Help
                  </a>
            ...
            {filtered.map(item => (
              <div key={item.id} style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.07), rgba(255,255,255,0.03))', borderRadius: '18px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.1)', transition: 'transform 0.2s, border-color 0.2s' }}
                ...
                <div style={{ padding: '16px' }}>
                  ...
                  <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', margin: '0 0 2px' }}>📍 {item.hostel}</p>
                  <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', margin: '0 0 12px' }}>📞 {item.phone} · 📧 {item.userEmail}</p>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    <span style={{ fontWeight: '800', color: '#c8f135', fontSize: '20px' }}>₹{item.price}</span>
                  </div>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <a href={"mailto:" + item.userEmail}
                      style={{ flex: 1, background: 'rgba(200,241,53,0.1)', color: '#c8f135', border: '1px solid rgba(200,241,53,0.2)', borderRadius: '8px', padding: '9px', fontSize: '13px', textDecoration: 'none', fontWeight: '600', textAlign: 'center' }}>
                      📧 Email
                    </a>
                    ...
                    <Link href={"/chat/" + item.id}
                      style={{ flex: 1, background: 'rgba(26,60,255,0.15)', color: '#7b8fff', border: '1px solid rgba(26,60,255,0.3)', borderRadius: '8px', padding: '9px', fontSize: '13px', textDecoration: 'none', fontWeight: '600', textAlign: 'center' }}>
                      💬 Chat
                    </Link>
                  </div>
                </div>
