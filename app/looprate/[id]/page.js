...
import { doc, getDoc, updateDoc, addDoc, getDocs, collection, serverTimestamp, query, orderBy, increment } from 'firebase/firestore';
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0d0d0d 0%, #1a1a2e 50%, #0d0d0d 100%)', fontFamily: "'Segoe UI', sans-serif", color: '#fff', paddingBottom: '100px' }}>
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto', padding: '20px' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.07), rgba(255,255,255,0.03))', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px', padding: '24px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center', marginBottom: '20px' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {faculty.subject && (
                <p style={{ margin: 0, fontSize: '13px', color: 'rgba(255,255,255,0.4)' }}>📚 {faculty.subject}</p>
              )}
            </div>
            {/* SCORE CIRCLE */}
            <div style={{ textAlign: 'center', flexShrink: 0 }}>
              ...
              <p style={{ margin: '6px 0 0', fontSize: '10px', color: scoreColor, fontWeight: '700', textAlign: 'center', maxWidth: '70px', lineHeight: '1.3' }}>{scoreLabel}</p>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '20px' }}>
            <div style={{ background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.2)', borderRadius: '12px', padding: '14px', textAlign: 'center' }}>
              ...
              <p style={{ margin: '4px 0 0', fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>🟢 Green</p>
            </div>
            <div style={{ background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: '12px', padding: '14px', textAlign: 'center' }}>
              ...
              <p style={{ margin: '4px 0 0', fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>🔴 Red</p>
            </div>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button onClick={() => vote('green')} disabled={!!voted || voting}
              style={{ flex: 1, padding: '14px', background: voted === 'green' ? 'rgba(74,222,128,0.3)' : 'rgba(74,222,128,0.1)', border: `2px solid ${voted === 'green' ? '#4ade80' : 'rgba(74,222,128,0.3)'}`, borderRadius: '12px', color: '#4ade80', fontWeight: '800', fontSize: '15px', cursor: voted ? 'not-allowed' : 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}>
              🟢 Green Flag {voted === 'green' && '✓'}
            </button>
            <button onClick={() => vote('red')} disabled={!!voted || voting}
              style={{ flex: 1, padding: '14px', background: voted === 'red' ? 'rgba(248,113,113,0.3)' : 'rgba(248,113,113,0.1)', border: `2px solid ${voted === 'red' ? '#f87171' : 'rgba(248,113,113,0.3)'}`, borderRadius: '12px', color: '#f87171', fontWeight: '800', fontSize: '15px', cursor: voted ? 'not-allowed' : 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}>
              🔴 Red Flag {voted === 'red' && '✓'}
            </button>
          </div>
          {voted && (
            <p style={{ textAlign: 'center', fontSize: '12px', color: 'rgba(255,255,255,0.4)', marginTop: '10px', marginBottom: 0 }}>
              You already voted for this faculty
            </p>
          )}
        </div>
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.06), rgba(255,255,255,0.02))', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', padding: '20px', marginBottom: '20px' }}>
          <h3 style={{ margin: '0 0 16px', fontSize: '16px', fontWeight: '800' }}>💬 Write a Review</h3>
          ...
          <textarea value={newReview} onChange={e => setNewReview(e.target.value)}
            placeholder="Share your experience with this faculty... Your review is anonymous"
            rows={3}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {reviews.map(review => (
              <div key={review.id} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '16px' }}>
                <p style={{ margin: '0 0 10px', fontSize: '12px', color: 'rgba(255,255,255,0.4)' }}>🎓 {review.userEmail}</p>
